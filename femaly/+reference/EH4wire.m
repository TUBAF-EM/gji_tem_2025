function F = EH4wire(tx, rx, sigma, depth, varargin)
    ...
    %
    % Beware the coordinate system definitions:
    ...
    parser_obj.addParameter('tx_type', 'E', @(x) ischar(x) && isscalar(x));
    parser_obj.addParameter('rx_type', 'dH', @(x) ischar(x) && (isscalar(x) || length(x) == 2));
    parser_obj.addParameter('t_type', 'time', @ischar);
    parser_obj.addParameter('t_list', 1, @isvector);
    parser_obj.addParameter('epsilon', [], @(x) isvector(x) && length(x) == length(sigma));
    parser_obj.addParameter('I', 1, @isscalar);
    parser_obj.addParameter('srcpts', 12, @(x) isscalar(x) && x > 0);
    parser_obj.addParameter('verb', 1, @(x) isscalar(x) && x < 5 && x >= 0);
    parser_obj.addParameter('sigma_air', 1e-14, @(x) isscalar(x) && x > 0);
    ...
    switch args.tx_type
        case 'H'
            msrc = true;
            warning('Receiver domain type not correctly defined yet!');
        case 'E'
            msrc = false;
        otherwise
