...
for dd = 1:length(tem_info.data_type)
    for pp = 1:size(tem_info.pt_RX, 1)
        ...
        % Get reference from VMD routine (verticalk dipole TX).
        [dBr, dBz] = deal(zeros(1, length(t_obs_)));
        for tt = 1:length(t_obs_)
            r = norm(point(pp, 1:2));
            if strcmp(tem_info.data_type{dd}, 'E')
                % 1./sig_earth, [], -point(pp, 3), ...
                dBr(:, tt) = reference.getVMDLayeredTransient(...
                                                        vmd_type, t_obs_(tt), r, ...
                                                         1./sig_earth, [], -point(pp, 3), ...
                                                         line(1, 3), 0, 1);
                dB_vmd = -[dBr * -point(pp, 2)/r; dBr * point(pp, 1)/r ; dBz];
            else
                [dBr(:, tt), dBz(:, tt)] = reference.getVMDLayeredTransient(...
                                                            vmd_type, t_obs_(tt), r, ...
                                                             1./sig_earth, [], -point(pp, 3), ...
                                                             line(1, 3), 0, 1);
                dB_vmd = -[dBr * -point(pp, 1)/r; dBr * -point(pp, 2)/r ; dBz] * scale;
            end
        end
        ...
        for cc = 1:3
            ...
            d_vmd = (dB_vmd(cc, :));                % at t_obs_
            leg_str = {'fe-', 'fe+', 'empy-', 'empy+', 'vmd-', 'vmd+'};
            leg_use = [any(d_fe < 0), any(d_fe > 0), ...
                       any(d_emp < 0), any(d_emp > 0), ...
                       any(d_vmd < 0), any(d_vmd > 0)];
            subplot(2, 3, cc)
                            % loglog(t_obs(d_fe < 0), abs(d_fe(d_fe < 0)), 'r.', ...
                semilogx(t_obs(d_fe < 0), abs(d_fe(d_fe < 0)), 'r.', ...
                         ...
                         t_obs_(d_vmd < 0), abs(d_vmd(d_vmd < 0)), 'rx', ...
                         t_obs_(d_vmd > 0), d_vmd(d_vmd > 0), 'bx');
