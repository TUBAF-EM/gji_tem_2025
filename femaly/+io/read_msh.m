...
function entity_markers = compute_entity_markers(mesh, ...
    ...
    if entity_dim == 0
        ...
        assert(~any(~entity_markers_entities), ...
            sprintf(['Some %dD entitie-to-vertex relations in file are ', ...
                     'not part of the mesh.'], entity_dim));
    end

    % Build sparse vector representation
    num_entities = mesh.num_entities(entity_dim);
    if numel(unique(entity_markers_entities)) ~= numel(entity_markers_entities)
        % Multiple markers for single entities.
        marker_values = unique(entity_markers_values);
        entity_marker_map = (entity_markers_values == marker_values.') .* ...
                            marker_values.';
        entity_markers = arrayfun(@(x) {sparse(entity_markers_entities, ...
                            1, entity_marker_map(:, x), num_entities, 1)}, ...
                            1:numel(marker_values));
        warning('read_msh:MultipleMarker', ...
                ['Found entities (dim = %d) with multiple markers.\n', ...
                 'Entities are stored in separate cells.'], ...
                entity_dim);
    else
        % Unique marker for single entities.
        entity_markers = sparse(entity_markers_entities, 1, ...
                                entity_markers_values, num_entities, 1);
    end
end
